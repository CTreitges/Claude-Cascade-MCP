...
_STRINGS: dict[str, dict[Lang, str]] = {
    ...
    "help": {
        "de": (
            "*Claude-Cascade Bot*\n\n"
            "Schicke Text/Voice/Foto/Dokument → Cascade-Run startet.\n\n"
            "/status [id]   — Task-Status\n"
            "/logs [id]     — letzte 50 Log-Zeilen\n"
            "/cancel [id]   — laufenden Task abbrechen\n"
            "/history       — letzte 10 Tasks\n"
            "/resume <id>   — unterbrochenen Task fortsetzen\n"
            "/repo <pfad>   — Default-Repo für diesen Chat (`clear` zum Löschen)\n"
        ...
        "en": (
            "*Claude-Cascade Bot*\n\n"
            "Send any text/voice/photo/document → cascade run.\n\n"
            "/status [id]   — task status\n"
